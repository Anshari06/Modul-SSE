...
<style>
    ...
    .stat-number.waiting { color: #93c5fd; }
    .stat-number.called  { color: #fcd34d; }
    .stat-number.done    { color: #6ee7b7; }
    ...
    @media (max-width: 768px) {
        .callout-number { font-size: 5rem; letter-spacing: -3px; }
        .callout-name { font-size: 1.4rem; }
...
<script>
    ...
    const serviceLabels = {
        'umum':     { text: 'Layanan Umum',     icon: 'bi-person-fill' },
        'prioritas':{ text: 'Layanan Prioritas',icon: 'bi-star-fill' },
        'bisnis':   { text: 'Layanan Bisnis',    icon: 'bi-briefcase-fill' },
    };
    ...
    function updateCallout(current) {
        ...
        const svc = serviceLabels[current.layanan] || { text: current.layanan, icon: 'bi-question-circle' };
    ...
    function updateNextQueues(queues) {
        ...
        if (waiting.length === 0) {
            el.innerHTML = '<div class="next-empty"><i class="bi bi-check-circle me-2"></i>Tidak ada antrian menunggu</div>';
            return;
        }
